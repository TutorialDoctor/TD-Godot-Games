...
func _ready():
	...
	status = Label.new()
	status.set_text('Sensing...')
	add_child(status)
	
	eyes = RayCast.new()
	eyes.set_enabled(true)
	eyes.set_cast_to(Vector3(0,0,-10))
	camera.add_child(eyes)
...
func see():
	if eyes.is_colliding():
		status.set_text('I see a ' + eyes.get_collider().get_name())
	else:
		status.set_text('Sensing...')
...
func lift():
	if Input.is_key_pressed(KEY_L):
		get_node("l_Arm/AnimationPlayer").play('move_up')
